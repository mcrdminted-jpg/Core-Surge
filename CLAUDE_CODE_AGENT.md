...
The in-game currency is called **Scrap**, not coins. All player-facing text, tooltips, labels, and UI strings must use "Scrap" (or the ⊙ icon). Internal code variables (`save.coins`, `coinRewardForRun`, etc.) can keep their current names since players never see them. Any NEW UI text referencing the currency must say "Scrap." Check `Tester Feedback/TESTER_FEEDBACK_LOG.md` for full tester feedback items ready for action.
