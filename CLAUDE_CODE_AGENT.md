...
- The build pipeline is already live in-repo and on Cloudflare Pages.
- Current tester URL: `https://core-surge.pages.dev/`
- Already complete in repo:
  - Build bundling/minification via `scripts/build.js`
  - `npm run build` output to `dist/`
  - `npm run test` exists and passes
  - `BUILD.md` exists
- Important nuance:
  - The live built service worker is generated by `scripts/build.js`
  - Do not assume the source `service-worker.js` alone represents the full live Pages behavior
- Next safe Claude Code lane:
  - Measure/document bundle baseline
  - Dead-code / dead-CSS audit
  - Asset optimization and lazy-loading
  - Lighthouse/perf work
- Do not re-claim build pipeline setup tasks unless you are improving them beyond the current implementation.
...
- [x] **Task 26:** Set up esbuild or webpack config - entry point main.js, output single minified bundle (verified complete in repo 2026-05-25)
- [x] **Task 27:** Configure CSS minification - combine all 7 CSS files into single core-surge-styles.min.css (verified complete in repo 2026-05-25)
- [x] **Task 28:** Create build script in package.json: `npm run build` outputs minified JS + CSS + assets to dist/ (verified complete in repo 2026-05-25)
...
- [x] **Task 50:** Document build pipeline in BUILD.md with step-by-step instructions for other developers (verified complete in repo 2026-05-25)
...
- [x] **Task 88:** Add npm test command to package.json (verified complete in repo 2026-05-25)
